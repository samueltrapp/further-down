...
import {ManeuverName, ManeuverType, OtherManeuverFnType, SelfManeuverFnType} from "../../types/maneuvers.ts";
...
export function alterStats(game: GameType, turn: TurnType, maneuverFns: StatEffects, maneuverDetails: ManeuverType): GameType {
    ...
    for (const action of maneuverDetails.actions) {
        ...
        const recipients = targetIds.map((targetId) => findCharacter(characters, targetId));
        recipients.forEach((receivingChar) => {
            const recipient = receivingChar[0];
            const recipientId = receivingChar[1];
            characters[recipientId] = {
                ...characters[recipientId],
                stats: maneuverFns.other(recipient.stats, {damage: modifiedDamage, damageType: action.damageType})
            }
        });
    }
